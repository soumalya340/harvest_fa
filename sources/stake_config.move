...
module harvest::stake_config {
    ...
    public entry fun initialize(
        emergency_admin: &signer, treasury_admin: address
    ) {
        assert!(
    ...
    public entry fun set_emergency_admin_address(
        emergency_admin: &signer, new_address: address
        ...
        assert!(
            signer::address_of(emergency_admin)
    ...
    public entry fun set_treasury_admin_address(
        treasury_admin: &signer, new_address: address
    ) acquires GlobalConfig {
        assert!(exists<GlobalConfig>(@stake_emergency_admin), ERR_NOT_INITIALIZED);
        let global_config = borrow_global_mut<GlobalConfig>(@stake_emergency_admin);
        assert!(
